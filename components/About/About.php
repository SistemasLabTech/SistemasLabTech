<div id="about" class="section wb">
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <div class="message-box">
                        <h4>Acerca De</h4>
                        <h2>Bienvenido a Maniobras Estratégicas del Pacifico S.A. de C.V.</h2>
                        <p class="lead">
Estamos comprometidos con nuestros clientes, empleados y proveedores, operando de manera segura, respeto al medio ambiente y cumplimiento legal asociado a las actividades de la compañía.</p>

                        <p> Maniobras Estratégicas del Pacifico S.A. de C.V. es una empresa dedicada a proporcionar servicios de almacenaje y maniobra en el puerto de Manzanillo. Contamos con una larga trayectoria en el mercado y cada uno de nuestros miembros  desempeña un papel importante para construir y defender la buena reputación, prácticas de ética empresarial e integridad de nuestros servicios. . </p>

                        <a href="#services" class="btn btn-light btn-radius btn-brd grd1">Acerca de</a>
                    </div><!-- end messagebox -->
                </div><!-- end col -->

                <div class="col-md-6">
                    <div class="post-media wow fadeIn">
                        <img src="assets/uploads/about_01.jpg" alt="" class="img-responsive img-rounded">
                        <a href="https://youtu.be/Ny-e8O8wJdU?si=1Gp8_A7OlwU8ouTO" data-rel="prettyPhoto[gal]" class="playbutton"><i class="flaticon-play-button"></i></a>
                    </div><!-- end media -->
                </div><!-- end col -->
            </div><!-- end row -->

            <hr class="hr1"> 

            <div class="row">
				<div class="col-md-6">
                    <div class="post-media wow fadeIn">
                        <img src="assets/uploads/about_02.jpg" alt="" class="img-responsive img-rounded">
                    </div><!-- end media -->
                </div><!-- end col -->
				
                <div class="col-md-6">
                    <div class="message-box">
                        <h4>Quienes somos</h4>
                        <h2>Maniobras Estratégicas del Pacifico S.A. de C.V.</h2>
                        <p class="lead">En Maniobras Estratégicas del Pacífico, S.A. de C.V. nace en el año 2003 con el objetivo de convertirse en una empresa especializada en logística internacional y comercio exterior, totalmente integrada para la planeación y desarrollo de procesos logísticos.</p>

                        <p> 
Procuramos acciones que permitan la mejora continua mediante la asignación de recursos necesarios y gestión de calidad, nos responsabilizamos de realizar negocios éticos, así como, de entregar servicios altamente satisfactorios a nuestros clientes.. </p>

                        <a href="#services" class="btn btn-light btn-radius btn-brd grd1">Acerca de</a>
                    </div><!-- end messagebox -->
                </div><!-- end col -->
            </div><!-- end row -->
        </div><!-- end container -->
    </div>